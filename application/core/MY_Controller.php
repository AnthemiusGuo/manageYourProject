...
class P_Controller extends CI_Controller {
    ...
    public $backInt = 0;
    public $resVersion = '2.0.2.1013';
    public $has_menu = true;
    public $createAsLightbox = true;
	...
	function __construct($login_verify = true,$userTyp='a') {
        ...
        $this->show_controller_name = $this->controller_name;
        $this->show_method_name = $this->method_name;
	...
	public function admin_load_menus(){
        ...
        array_unshift($this->title,$this->menus[$this->show_controller_name]['menu_array'][$this->show_method_name]['name']);
	}
	...
	function login_verify($force=true) {
		...
		if ($init_result<0){
            ...
            $this->is_login = true;
            if (isset($this->userInfo->field_list['projectId'])){
                $this->currentProjectId = $this->userInfo->field_list['projectId'];
	...
	public function genBreadCrumb(){
		...
		<li><a><span class='glyphicon {$this->Menus->show_menus[$this->controller_name]['icon']}'></span> {$this->Menus->show_menus[$this->controller_name]['name']}</a></li>
  ...
  //   	$config['protocol'] = 'smtp';
    ...
    public function sendRtxNotify($uids,$title,$msg,$url="",$delaytime=0){
        return;
        $this->load->model('lists/User_list',"notifyUserList");
        $this->load->model('records/Email_model',"emailInfo");
        foreach ($uids as $key => $value) {
            if (!is_object($value)){
                $uids[$key] = new MongoId($value);
            }
            $data=array();
            $data['name']=$title;
		...
		$this->notifyUserList->add_where(WHERE_TYPE_IN,"_id",$uids);
		$this->notifyUserList->load_data_with_where();

        $loginNames = $this->notifyUserList->gen_value_array('loginName');
        //$recivers,$title,$msg,$url="",$delaytime=0
        $this->load->library('Rtx');

        $this->rtx->sendNotify($loginNames,$title,$msg,site_url($url),$delaytime);
